//disables warnings that private fields are not assigned to, 
//since they are serialized and assigned from the inspector
...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarController : MonoBehaviour
    {
        ...
        [SerializeField]
        private float m_TopReversespeed = 12;
        [SerializeField]
        ...
        [SerializeField]
        private bool useChangingDampeningRate;
        [SerializeField]
        private float dampRateAtMinSpeed, dampRateAtMaxSpeed;
        ...
        {
            float damp = 0;
            if (useChangingDampeningRate)
                damp = Mathf.Lerp(dampRateAtMinSpeed, dampRateAtMaxSpeed, CurrentSpeed / MaxSpeed);
            for (int i = 0; i < 4; i++)
            {
                Quaternion quat;
                Vector3 position;
                ...
                m_WheelMeshes[i].transform.rotation = quat * Quaternion.Inverse(m_WheelMeshLocalRotations[i]);
                if (useChangingDampeningRate)
                    m_WheelColliders[i].wheelDampingRate = damp;
            }

            //clamp input values
            steering = Mathf.Clamp(steering, -1, 1);
            AccelInput = accel = Mathf.Clamp(accel, 0, 1);
            //for some reason footbrake is reverse.
            BrakeInput = footbrake = -1 * Mathf.Clamp(footbrake, -1, 0);
            //hand break is the break.
        ...
        private void CapSpeed()
        {
            ...
            float angle = Vector3.Angle(m_Rigidbody.velocity.normalized, transform.forward);
            switch (m_SpeedType)
            {
                case SpeedType.MPH:
                ...
                else if (speed > m_TopReversespeed)
